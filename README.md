...
- **AI**:
  - **작품 기본정보 자동입력** → Google Gemini API (실시간 검색으로 최신 공연 정보 조회)
...
**(a) Gemini — 작품 정보 자동입력용 (실시간 검색)**

1. [Google AI Studio](https://aistudio.google.com/apikey)에서 API 키(`AIza…`)를 발급받습니다.
...
| `GEMINI_API_KEY` | 2-(a)에서 발급한 Gemini API 키 (`AIza…`) |
...
- 새 기록에서 작품명을 입력하고 **AI 자동입력**을 누르면 Gemini가 실시간 검색으로
...
- **Gemini·Groq API 키는 정적 사이트에 주입되므로 방문자가 볼 수 있습니다.**
  Gemini 키는 Google AI Studio에서 웹사이트(HTTP 리퍼러) 제한을 걸고,
  Groq 키는 리퍼러 제한을 걸 수 없으니 무료 등급 전용 키를 쓰고 사용량을 주기적으로 확인하세요.
  두 키 모두 무료 할당량 내에서 사용하고, 노출되어 문제가 되면 즉시 폐기·재발급하면 됩니다.
