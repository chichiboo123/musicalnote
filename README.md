...
- **AI**: Groq API — 작품 기본정보 자동입력, 후기 글 다듬기 (여러 모델 자동 폴백)
...
2. AI 호출은 아래 순서로 시도하고, 앞 모델이 실패(한도 초과·점검 등)하면 자동으로 다음 모델로 넘어갑니다.
...
| `GROQ_API_KEY` | 2번에서 발급한 Groq API 키 (`gsk_…`) |
...
- 새 기록에서 작품명을 입력하고 **AI 자동입력**을 누르면 Groq 모델이 아는 정보로
  공연장·러닝타임·시놉시스·캐스트·포스터를 채워줍니다. (실시간 웹 검색은 하지 않으므로
  최신·비주류 작품은 빈 칸이 있을 수 있고, 채워진 내용은 확인 후 사용하세요.)
...
- **Groq API 키는 정적 사이트에 주입되므로 방문자가 볼 수 있습니다.**
  ...
  키가 노출되어 문제가 되면 Groq Console에서 즉시 폐기하고 재발급하면 됩니다.
